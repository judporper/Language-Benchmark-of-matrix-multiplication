...
public class MatrixMultiplier {
	...
	public static double[][] multiplyParallelRows(double[][] A, double[][] B, int threads) throws InterruptedException {
		int n = A.length;
		double[][] C = new double[n][n];
	...
	public static double[][] multiplyBlockedParallel(double[][] A, double[][] B, int blockSize, int threads) throws InterruptedException {
		int n = A.length;
		double[][] C = new double[n][n];
		ExecutorService pool = Executors.newFixedThreadPool(threads);
		...
		for (int ii=0; ii<n; ii+=blockSize) {
			...
			for (int jj=0; jj<n; jj+=blockSize) {
				...
				pool.submit(() -> {
					for (int kk=0; kk<n; kk+=blockSize) {
						int iimax = Math.min(fii + blockSize, n);
						int jjmax = Math.min(fjj + blockSize, n);
						int kkmax = Math.min(kk + blockSize, n);
						for (int i=fii; i<iimax; i++) {
							for (int j=fjj; j<jjmax; j++) {
								double sum = 0.0;
	...
	public static class CSRMatrix {
		...
		public void multiplyParallel(double[] x, double[] y, int threads) {
